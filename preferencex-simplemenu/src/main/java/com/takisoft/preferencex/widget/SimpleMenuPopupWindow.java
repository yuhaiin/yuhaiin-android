...
public class SimpleMenuPopupWindow extends PopupWindow {
    ...
    public SimpleMenuPopupWindow(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        RecyclerView mList = (RecyclerView) LayoutInflater.from(context).inflate(R.layout.simple_menu_list, null);
    ...
    private void showDialog(View parent, View container) {
        ...
        getContentView().post(() -> {
            int width1 = getContentView().getWidth();
            int height = getContentView().getHeight();
            Rect start = new Rect(width1 / 2, height / 2, width1 / 2, height / 2);

            SimpleMenuAnimation.startEnterAnimation(getContentView(), getBackground(),
                    width1, height, width1 / 2, height / 2, start, itemHeight, elevation[DIALOG] / 4, index);
        });
